import type { DoGet, Response } from '../types/types.ts';
...
export async function doGet({
	...
	if (columnTitles) {
		URLParams.append('columnTitles', JSON.stringify(columnTitles));
	}
	if (columnIndexes) {
		URLParams.append('columnIndexes', JSON.stringify(columnIndexes));
	}
